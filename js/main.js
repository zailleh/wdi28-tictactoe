...
const diag2ForWin = function ( player ) {
  ...
  if (aiEnabled === true && winSlots.length === 2 && gameData[x][y] === null) {
      Array.prototype.push.apply(aiSlots, emptySlots);
  }
  if(winSlots.length === 3) {
    win = true;
...
const unbeatableAI = function () {
  ...
  let win = checkYforWin('naught');
  if (win.aiSlots.length > 0) {
    aiClickTrigger( win );
    return;
  }

  win = checkXforWin('naught');
  if (win.aiSlots.length > 0) {
    aiClickTrigger( win );
    return;
  }

  win = diag1ForWin('naught');
  if (win.aiSlots.length > 0) {
    aiClickTrigger( win );
    return;
  }

  win = diag2ForWin('naught');
  if (win.aiSlots.length > 0) {
    aiClickTrigger( win );
    return;
  }

  //no danger of losing found above.
  let freeSlot = false;
  do {
    win.aiSlots = [{
      x: getRandomInt(3),
